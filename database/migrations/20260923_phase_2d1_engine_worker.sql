...
ALTER TABLE automation_events
  ADD COLUMN IF NOT EXISTS engine_status ENUM('pending', 'processing', 'processed', 'failed') NOT NULL DEFAULT 'pending' AFTER causation_id,
  ADD COLUMN IF NOT EXISTS engine_locked_at DATETIME NULL AFTER engine_status,
  ADD COLUMN IF NOT EXISTS engine_locked_by VARCHAR(100) NULL AFTER engine_locked_at,
  ADD COLUMN IF NOT EXISTS engine_attempts INT UNSIGNED NOT NULL DEFAULT 0 AFTER engine_locked_by,
  ADD COLUMN IF NOT EXISTS engine_last_error TEXT NULL AFTER engine_attempts,
  ADD COLUMN IF NOT EXISTS engine_processed_at DATETIME NULL AFTER engine_last_error;
