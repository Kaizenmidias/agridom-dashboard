...
function FreeformAutomationBuilder(props: {
  ...
  onPublish?: (definition?: AutomationDefinition) => Promise<void>;
  readOnly?: boolean;
}) {
  ...
  const save = async (notify = true) => { if (!props.draft) { toast.error("Crie um rascunho antes de salvar o fluxo."); return false; } setSaving(true); try { await automationsAPI.updateVersion(props.automationId, props.draft.id, currentDefinition()); setDirty(false); if (notify) toast.success("Rascunho salvo."); await props.onSaved(); return true; } catch (error) { toast.error(error instanceof Error ? error.message : "Não foi possível salvar o rascunho."); return false; } finally { setSaving(false); } };
  const publishCurrent = async () => { if (!props.onPublish) return; if (!props.draft) { await props.onPublish(); return; } setSaving(true); try { const latest = currentDefinition(); await automationsAPI.updateVersion(props.automationId, props.draft.id, latest); setDirty(false); await props.onPublish(latest); } catch (error) { toast.error(error instanceof Error ? error.message : "Não foi possível publicar o fluxo."); } finally { setSaving(false); } };
