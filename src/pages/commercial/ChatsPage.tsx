...
import { Bot, Check, CheckCheck, ChevronDown, CircleUserRound, Clock3, Copy, Inbox, Instagram, Linkedin, MessageCircle, MoreHorizontal, Pause, Phone, RefreshCw, Search, Send, UserRound, UserRoundCheck } from "lucide-react";
...
export function ChatsPage() {
  ...
  const loadSelected = useCallback(async (id: number, silent = false) => {
    ...
    try {
      ...
      setDetail(nextDetail); setMessages(nextMessages.messages); setActivities(nextActivities.activities);
  ...
  const send = async () => { if (!detail || !text.trim() || sending) return; setSending(true); try { await conversationsAPI.send(detail.id, text.trim()); setText(""); await loadSelected(detail.id, true); await loadConversations(true); composerRef.current?.focus(); } catch (requestError) { setError(requestError instanceof Error ? requestError.message : "Não foi possível enviar a mensagem."); } finally { setSending(false); } };
  ...
  return <TooltipProvider>
    <main className="flex h-[calc(100vh-5.5rem)] min-h-[620px] flex-col overflow-hidden p-3 md:p-5">
      ...
      <div className="grid min-h-0 flex-1 overflow-hidden rounded-lg border bg-card shadow-sm lg:grid-cols-[300px_minmax(0,1fr)_320px]">
        ...
        <section className="flex min-h-0 flex-col border-b lg:border-b-0 lg:border-r">{detail ? <><header className="flex shrink-0 items-center justify-between gap-3 border-b px-4 py-3"><div className="flex min-w-0 items-center gap-3"><Avatar className="h-10 w-10"><AvatarFallback>{initials(detail.lead_name || detail.lead_phone)}</AvatarFallback></Avatar><div className="min-w-0"><h2 className="truncate font-semibold">{detail.lead_name || detail.lead_phone || "Contato sem nome"}</h2><div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><ChannelBadge /><span>•</span><span>{detail.lead_phone || "Sem telefone"}</span></div></div></div><div className="flex items-center gap-1"><Badge variant="outline" className={cn(detail.handling_mode === "ai" ? "border-sky-500/40 text-sky-600" : detail.handling_mode === "paused" ? "border-amber-500/40 text-amber-600" : "border-primary/40 text-primary")}><span className="mr-1.5 h-1.5 w-1.5 rounded-full bg-current" />{modeLabel(detail.handling_mode)}</Badge><Button variant="ghost" size="icon" onClick={changeHandling} title={detail.handling_mode === "ai" ? "Assumir atendimento" : detail.handling_mode === "human" ? "Pausar atendimento" : "Assumir atendimento"} aria-label="Alterar modo de atendimento">{detail.handling_mode === "ai" ? <UserRoundCheck className="h-4 w-4" /> : detail.handling_mode === "paused" ? <UserRound className="h-4 w-4" /> : <Pause className="h-4 w-4" />}</Button><Button variant="ghost" size="icon" title="Mais ações" aria-label="Mais ações"><MoreHorizontal className="h-4 w-4" /></Button></div></header><div className="min-h-0 flex-1 overflow-y-auto bg-muted/20 px-4 py-5">{loadingMessages ? <div className="space-y-3"><Skeleton className="h-12 w-2/3" /><Skeleton className="ml-auto h-16 w-1/2" /><Skeleton className="h-10 w-1/3" /></div> : messages.length === 0 ? <EmptyState icon={MessageCircle} title="Nenhuma mensagem nesta conversa" /> : <div className="mx-auto flex max-w-3xl flex-col gap-2">{messages.map((message, index) => <MessageBubble key={message.id} message={message} previous={messages[index - 1]} />)}</div>}</div><div className="shrink-0 border-t bg-card p-3"><div className="flex items-end gap-2 rounded-md border bg-background p-2 focus-within:border-primary/60"><Button variant="ghost" size="icon" disabled title="Anexos estarão disponíveis na próxima fase" aria-label="Anexos"><MoreHorizontal className="h-4 w-4" /></Button><Textarea ref={composerRef} value={text} onChange={(event) => setText(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void send(); } }} placeholder="Digite uma mensagem..." className="min-h-10 resize-none border-0 bg-transparent p-2 shadow-none focus-visible:ring-0" rows={1} /><Button size="icon" onClick={() => void send()} disabled={!text.trim() || sending} aria-label="Enviar mensagem" title="Enviar mensagem">{sending ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}</Button></div><p className="mt-1 px-2 text-[11px] text-muted-foreground">Enter envia · Shift+Enter quebra linha</p></div></> : <EmptyState icon={MessageCircle} title="Selecione uma conversa para começar" />}</section>
