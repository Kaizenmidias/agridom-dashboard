...
export type CommunicationMessage = { id: number; direction: string; body_text?: string | null; delivery_status?: string | null; message_type?: string | null; created_at: string; sent_at?: string | null; provider?: string | null };
...
export const conversationsAPI = {
  ...
  messages: (id: number) => request<{ messages: CommunicationMessage[]; hasMore?: boolean }>(`/api/conversations/${id}/messages`),
  ...
  send: (id: number, text: string) => request(`/api/conversations/${id}/messages`, { method: 'POST', headers: { 'Idempotency-Key': `chat:${id}:${Date.now()}` }, body: JSON.stringify({ type: 'text', text }) }),
};
